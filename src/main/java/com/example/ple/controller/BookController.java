...
public class BookController {
  ...
  @Autowired
  public BookController(CommandBus commandBus, EventStore eventStore) {
    this.commandBus = commandBus;
    this.eventStore = eventStore;
  }

    @PostMapping("/registerbook")
    public Map<String, Object> registerBook(@RequestBody(required = false) Map<String, Object> payload) {
        ...
        List<DomainEvent> events = commandBus.dispatch("Book", "RegisterBook", bookId, actualPayload);
        eventStore.save(bookId, events);
        return projectState(bookId, events);
    }

    @PostMapping("/{bookId}/markbookasrestricted")
    public Map<String, Object> markBookAsRestricted(@PathVariable String bookId, @RequestBody(required = false) Map<String, Object> payload) {
        Map<String, Object> actualPayload = payload != null ? payload : new java.util.HashMap<>();
        List<DomainEvent> events = commandBus.dispatch("Book", "MarkBookAsRestricted", bookId, actualPayload);
        eventStore.save(bookId, events);
        return projectState(bookId, events);
    }
    ...
    @GetMapping("/{bookId}")
    public ResponseEntity<?> getById(@PathVariable String bookId) {
        List<DomainEvent> events = eventStore.loadAll(bookId);
        if (events.isEmpty()) {
            Map<String, Object> error = new HashMap<>();
            error.put("error", "AggregateNotFound");
            error.put("aggregateType", "Book");
            error.put("bookId", bookId);
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(error);
        } else {
            return ResponseEntity.ok(projectState(bookId, events));
        }
    }
  ...
  private Map<String, Object> projectState(String id, List<DomainEvent> events) {
    ...
    state.put("title", null);
    state.put("restricted", null);
    ...
    for (DomainEvent event : events) {
      ...
      try {
        ...
        if (payload != null) {
          ...
          if (payload.containsKey("title")) { state.put("title", payload.get("title")); }
          if (payload.containsKey("restricted")) { state.put("restricted", payload.get("restricted")); }
